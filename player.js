...
Player.prototype = {
	...
	$$: function(name, dom) {
		...
		if (dom.id == name || dom.className == name) {
			return dom;
		}
	...
	on: function(type, fun) {
		this.elm.addEventListener(type,fun);
		return this;
	},
	hide: function() {
	...
	init: function(name, time) {
		...
		this.$('player_volume_icon').on("click",
		function(e) {
			if (_that.volume > 0) {
	...
	disabled: function() {
		this.$('player_pop_layer').show();
	},
	
	enabled: function() {
		this.$('player_pop_layer').hide();
	},
	...
	getSlider: function(name, e) {
		...
		var width = this.getwidth(slider);
	...
	updatePlayButton: function() {
		if (this.paused) this.$('player_playback_pause').elm.className = "player_pause";
		else this.$('player_playback_pause').elm.className = "player_playback";
	},

	updateTimerUI: function() {
		this.$("player_timer").elm.innerHTML = this.formatTime(Math.round(this.currentTime)) + " / " + this.formatTime(Math.round(this.duration));
	},

	updateProgressUI: function() {
		this.$("player_progress_loaded").elm.style.width = Math.round((this.currentTime / this.duration) * 100) + "%";
	},

	updateVolumeUI: function() {
		this.$("player_volume_loaded").elm.style.width = this.volume + "%";
		if (this.volume == 0) this.$('player_speaker_iocn_disabled').show();
		else this.$('player_speaker_iocn_disabled').hide();
	},
